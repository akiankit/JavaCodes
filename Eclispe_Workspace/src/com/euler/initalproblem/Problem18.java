...
public class Problem18 {
	...
	private static String fileName = "D:\\workspace\\temp\\src\\projectEuler\\problem18Input";

	public static void main(String[] args) throws Exception {
		String[] data = FileUtil.getDataFromFile(fileName);
		int[][] input = new int[data.length][];
		for (int i = 0; i < data.length; i++) {
			String[] numbers = data[i].split("\\s+");
			int[] temp = new int[numbers.length];
			for (int j = 0; j < numbers.length; j++) {
				temp[j] = Integer.parseInt(numbers[j]);
			}
			input[i] = temp;
		}
		/*
		 * for(int i=0;i<input.length;i++){ for (int j = 0; j < input[i].length;
		...
		for (int i = output.length - 2; i >= 0; i--) {
			for (int j = 0; j < output[i].length; j++) {
				output[i][j] = Math.max(input[i + 1][j] + input[i][j], input[i][j] + input[i + 1][j + 1]);
			}
		}
